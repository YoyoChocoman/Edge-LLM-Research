...
def analyze_nctx_results(json_path, window_size=20):
    ...
    for key, item in results.items():
        ...
        if not valid_positions:
            prompt_groups[prompt_name][n_ctx] = {
    ...
    for prompt_name, n_ctx_dict in sorted(prompt_groups.items()):
        print("=" * 110)
        print(f"PROMPT: {prompt_name}")
        print("=" * 110)

        # Part 1: Spike Locations Comparison
        print("[Part 1: Spike Locations Comparison across n_ctx]")
        print(f"{'Experiment':<18} | {'In Toks':<7} | {'Spike Chunk':<11} | {'Total Ctx':<9} | {'Spike P50':<10} | {'Content'}")
        print("-" * 100)

        for n_ctx, p_data in sorted(n_ctx_dict.items()):
            exp_name = f"n_ctx: {n_ctx}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="n_ctx Comprehensive Spike & Step-Up Analysis")
    parser.add_argument("json_file", help="Path to the n_ctx JSON results file")
    args = parser.parse_args()

    analyze_nctx_results(args.json_file)