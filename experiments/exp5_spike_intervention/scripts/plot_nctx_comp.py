...
def plot_nctx_grid(json_path):
    ...
    for key, item in results.items():
        ...
        prompt_groups[prompt_name][n_ctx] = item

    all_prompts = sorted(list(prompt_groups.keys()))
    all_n_ctx = sorted(list(set(item.get("n_ctx") for item in results.values())))

    num_prompts = len(all_prompts)
    num_ctxs = len(all_n_ctx)

    if num_prompts == 0 or num_ctxs == 0:
        print("No valid data found to plot.")
        return

    fig, axes = plt.subplots(num_prompts, num_ctxs, figsize=(5 * num_ctxs, 4 * num_prompts), sharex=False, sharey=True)
    fig.suptitle("Per-Chunk Streaming Latency Dynamics by n_ctx Settings", fontsize=16, y=0.98)

    if num_prompts == 1 and num_ctxs == 1:
        axes = np.array([[axes]])
    elif num_prompts == 1:
        axes = np.array([axes])
    elif num_ctxs == 1:
    ...
    for row_idx, prompt_name in enumerate(all_prompts):
        for col_idx, n_ctx_val in enumerate(all_n_ctx):
            ax = axes[row_idx, col_idx]
            item = prompt_groups[prompt_name].get(n_ctx_val, {})
            ...
            if not position_deltas:
                ax.set_title(f"[{prompt_name}] n_ctx: {n_ctx_val}\nNo Data", fontsize=10)
                continue
    ...
    output_path = "figures/nctx_grid_comparison.png"
    plt.tight_layout(rect=[0.02, 0.03, 1, 0.96])
    plt.savefig(output_path, dpi=300)
    print(f"Grid comparison plot successfully saved to {output_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="n_ctx Grid Latency Plotter")
    parser.add_argument("json_file", help="Path to the n_ctx JSON results file")
    args = parser.parse_args()

    plot_nctx_grid(args.json_file)