...
MAX_TOKENS = 500
RUNS_PER_SETTING = 20
N_BATCH_VALUES = [128, 256, 512, 1024]
RESULTS_FILE = f"results/nbatch_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    experiment_data = {"metadata": {"experiment": "n_batch Intervention"}, "results": {}}

    print("Starting n_batch Intervention Benchmark (Multi-Prompt)...")
    print("-" * 60)

    for n_batch_val in N_BATCH_VALUES:
        gc.collect()
        ...
        for prompt_name, prompt_text in PROMPTS.items():
            print(f"Running n_batch={n_batch_val} | {prompt_name}...")
            ...
            key_name = f"n_batch_{n_batch_val}_{prompt_name}"
